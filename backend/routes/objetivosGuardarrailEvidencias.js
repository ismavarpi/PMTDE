...
async function recalcEvidencias(objetivoId) {
  const pool = getDb();
  const [[obj]] = await pool.query('SELECT codigo FROM objetivos_guardarrail WHERE id=?', [objetivoId]);
...
router.get('/', async (req, res) => {
  ...
  const pool = getDb();
  const [rows] = await pool.query('SELECT id, codigo, descripcion FROM objetivos_guardarrail_evidencias WHERE objetivo_id=?', [objetivoId]);
  res.json(rows);
});

router.post('/', async (req, res) => {
  const pool = getDb();
  const objetivoId = req.body.objetivo && req.body.objetivo.id ? req.body.objetivo.id : 1;
  const descripcion = req.body.descripcion || 'n/a';
  const id = req.body.id;
  if (id) {
    await pool.query('UPDATE objetivos_guardarrail_evidencias SET descripcion=? WHERE id=?', [descripcion, id]);
...
router.delete('/:id', async (req, res) => {
  ...
  const [[row]] = await pool.query('SELECT objetivo_id FROM objetivos_guardarrail_evidencias WHERE id=?', [id]);
  if (!row) return res.sendStatus(404);
  if (req.query.confirm !== 'true') {
    return res.status(400).json({ message: 'Confirmar eliminación' });
  }
  await pool.query('DELETE FROM objetivos_guardarrail_evidencias WHERE id=?', [id]);
